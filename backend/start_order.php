...
if (isset($active_log) && $active_log == 1) {
    ...
    $stmt = $con->prepare("SELECT * FROM " . $siteprefix . "orders WHERE user = ? AND status = 'unpaid' ORDER BY date DESC LIMIT 1");
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $con->prepare("INSERT INTO " . $siteprefix . "orders (order_id, user, status, total_amount, date) VALUES (?, ?, 'unpaid', '0', ?)");
